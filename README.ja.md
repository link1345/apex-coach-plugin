...
レビュー開始時に動画解析ランタイムを診断し、最終回答前に構造化した判断と読者向け文面を検証します。実行不能なアビリティ、出典のない数値閾値、部隊HUDの不正な集約、根拠のない敵移動の断定、検証後に追加された数値、未確認音声への依存、曖昧な行動の断定比較、回復条件や参照情報の過剰主張は検証エラーになります。検証カバレッジでは、検証済みfindingと補足観察を区別します。
